make_route_module <- function(root, id, yaml, seq = "ATGGCTTAA", nested = FALSE) {
...
test_that("external module registry deduplicates and prefers top-level records", {
  ...
  make_route_module(root, "DupReporter", yaml, nested = TRUE)
  make_route_module(root, "DupReporter", yaml, nested = FALSE)
...
test_that("HDR and MMEJ route verdicts cover compact and repeat-blocked modules", {
  root <- file.path(tempdir(), paste0("forgeki_route_verdict_", as.integer(runif(1, 1, 1e9))))
  dir.create(root, recursive = TRUE, showWarnings = FALSE)
  make_route_module(root, "CompactReporter", list(
  ...
  make_route_module(root, "RepeatReporter", list(
...
test_that("route compatibility is visible through forgeki_available_modules and route helper", {
  root <- file.path(tempdir(), paste0("forgeki_route_available_", as.integer(runif(1, 1, 1e9))))
  dir.create(root, recursive = TRUE, showWarnings = FALSE)
  make_route_module(root, "AvailableReporter", list(
